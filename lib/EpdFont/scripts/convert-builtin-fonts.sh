...
LEXICA_FONT_SIZES=(10 12 14 16 18)
...
# Note: Lexica Ultralegible covers Latin scripts only (no Cyrillic/CJK).
# Cyrillic readers should use Bookerly, NotoSans, or OpenDyslexic.
for size in ${LEXICA_FONT_SIZES[@]}; do
  for style in ${READER_FONT_STYLES[@]}; do
    font_name="lexica_${size}_$(echo $style | tr '[:upper:]' '[:lower:]')"
    font_path="../builtinFonts/source/LexicaUltralegible/LexicaUltralegible-${style}.otf"
    output_path="../builtinFonts/${font_name}.h"
    python fontconvert.py $font_name $size $font_path --2bit --compress > $output_path
    echo "Generated $output_path"
  done
